...
class _TaggedConfigLoader(yaml.SafeLoader):
    """
...
def _construct_python_object(loader: yaml.SafeLoader, suffix: str, node: yaml.Node):
...
def _construct_python_apply(loader: yaml.SafeLoader, suffix: str, node: yaml.Node):
    ...
    raise ConstructorError(
        None,
        None,
        f"Blocked unsafe python/object/apply tag: {suffix}",
        node.start_mark,
    )


_TaggedConfigLoader.add_constructor(
    "tag:yaml.org,2002:python/tuple",
    _construct_python_tuple,
)
_TaggedConfigLoader.add_multi_constructor(
...
def load_config(src_folder: Path):
    ...
    cfg = yaml.load(config_path.read_text(), Loader=_TaggedConfigLoader)
    # assert isinstance(cfg, BaseTrackingConfig)
    return src_folder, cfg


def _coerce_dataclass(raw: dict, cls):
    allowed = {f.name for f in fields(cls)}
    payload = {k: v for k, v in raw.items() if k in allowed}
    for key, value in list(payload.items()):
        if key.endswith("_folder") or key.endswith("_path"):
...
def main(
    ...
    cfg_data_raw = cfg["data"] if isinstance(cfg, dict) else cfg.data
    cfg_model_raw = cfg["model"] if isinstance(cfg, dict) else cfg.model
    cfg_data = cfg_data_raw if isinstance(cfg_data_raw, DataConfig) else _coerce_dataclass(cfg_data_raw, DataConfig)
    cfg_model = cfg_model_raw if isinstance(cfg_model_raw, ModelConfig) else _coerce_dataclass(cfg_model_raw, ModelConfig)
    ...
    nerf_dataset_writer = NeRFDatasetWriter(cfg_data, tgt_folder, subset, scale_factor, background_color)
    nerf_dataset_writer.write()

    flame_dataset_writer = TrackedFLAMEDatasetWriter(cfg_model, src_folder, tgt_folder, mode=flame_mode, epoch=epoch)
    flame_dataset_writer.write()

    if create_mask_from_mesh:
        mask_generator = MaskFromFLAME(cfg_data, cfg_model, tgt_folder, background_color)
