...
class FlameTracker:
    ...
    def load_from_tracked_flame_params(self, fp):
        ...
        """
        report = np.load(fp)
        if "n_processed_frames" in report and self.cfg.begin_timestep == 0:
            self.cfg.begin_timestep = int(report["n_processed_frames"])
    ...
    def save_result(self, fname=None, epoch=None):
        ...
        output_path = self.out_dir / f'{fname}.npz'
        tmp_path = output_path.with_name(f"{output_path.name}.tmp")
        with open(tmp_path, "wb") as f:
            np.savez(f, **export_dict)
...
class GlobalTracker(FlameTracker):
    ...
    def optimize(self):
        ...
        last_checkpoint_timestep = self.cfg.begin_timestep
        ...
        for sample in dataloader:
            sample_start = sample["timestep_index"][0].item()
            sample_end = sample["timestep_index"][-1].item()
            if sample_start < self.cfg.begin_timestep:
                continue
            ...
            self.timestep = max(self.timestep, sample_end + 1)
            if self.timestep - last_checkpoint_timestep >= 500:
                self.save_result(fname="checkpoint_flame_params_latest")
                self.save_result(fname=f"checkpoint_flame_params_{self.timestep:05d}")
                self.logger.info(f"Saved restart checkpoint after {self.timestep} frames")
                last_checkpoint_timestep = self.timestep
