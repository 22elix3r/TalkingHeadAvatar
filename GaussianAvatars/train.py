...
def _camera_loader(cameras, shuffle, num_workers):
    loader_kwargs = {
        "batch_size": None,
        "shuffle": shuffle,
    ...
    return DataLoader(cameras, **loader_kwargs)

def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, data_loader_workers):
    first_iter = 0
    ...
    if dataset.bind_to_mesh:
        ...
        gaussians.enable_partwise_deformation = bool(getattr(dataset, "enable_partwise_deformation", False))
        gaussians.enable_mouth_structure = bool(getattr(dataset, "enable_mouth_structure", False))
    else:
    ...
    loader_camera_train = _camera_loader(scene.getTrainCameras(), shuffle=True, num_workers=data_loader_workers)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if dataset.bind_to_mesh:
            # GeoAvatar APS: activate rigid/flexible classification at 30K
            if dataset.enable_aps and iteration == 30_000:
                print("[APS] Running Adaptive Pre-allocation Stage...")
                gaussians.adaptive_preallocate()
                gaussians.lambda_rigid = opt.lambda_rigid
                gaussians.lambda_flex = opt.lambda_flex

            # Mouth structure + part groups at 50K
            if iteration == 50_000 and (gaussians.enable_partwise_deformation or gaussians.enable_mouth_structure):
                if gaussians.enable_mouth_structure:
                    print("[Mouth] Constructing inner-mouth structure...")
                    gaussians.mouth_structure = gaussians.flame_model.construct_mouth_structure()
                print("[Mouth] Assigning mouth part groups...")
                gaussians.assign_mouth_part_groups()
        ...
        if gaussians.binding != None:
            if iteration >= 50_000 and (gaussians.enable_partwise_deformation or gaussians.enable_mouth_structure):
                if gaussians.mouth_group is None or gaussians.mouth_group.shape[0] != gaussians._xyz.shape[0]:
                    gaussians.assign_mouth_part_groups()
        ...
        if gaussians.binding != None:
            ...
            # GeoAvatar Phase 3.4: rigging regularization loss. Densification
            # changes the Gaussian count, so APS masks must be refreshed when
            # resuming after APS or after clone/split/prune updates.
            if dataset.enable_aps and iteration >= 30_000:
                if gaussians.rigid_mask is None or gaussians.rigid_mask.shape[0] != gaussians._xyz.shape[0]:
                    gaussians.adaptive_preallocate()
                rig_loss = gaussians.compute_rigging_loss()
                if rig_loss.item() > 0.0:
                    losses['rig'] = rig_loss
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, losses, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), data_loader_workers)
...
def training_report(tb_writer, iteration, losses, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, data_loader_workers):
    if tb_writer:
        ...
        if 'rig' in losses:
            tb_writer.add_scalar('train_loss_patches/rig_loss', losses['rig'].item(), iteration)
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                gt_image_cache = []
                vis_ct = 0
                for idx, viewpoint in tqdm(enumerate(_camera_loader(config['cameras'], shuffle=False, num_workers=data_loader_workers)), total=len(config['cameras'])):
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.data_loader_workers)
