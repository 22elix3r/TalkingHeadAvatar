...
class GaussianModel:
    ...
    def _invalidate_cached_geometry(self):
        if hasattr(self, "_deformed_world_xyz"):
            self._deformed_world_xyz = None
        if hasattr(self, "_group_reference_size"):
            self._group_reference_size = 0

    def _resize_densification_stats(self, num_points):
        def resize_tensor(tensor, shape):
            ...
            if tensor.shape[0] == num_points:
                return tensor
            if tensor.shape[0] > num_points:
                return tensor[:num_points]
            pad_shape = (num_points - tensor.shape[0], *tensor.shape[1:])
            return torch.cat((tensor, torch.zeros(pad_shape, dtype=tensor.dtype, device=tensor.device)), dim=0)

        self.xyz_gradient_accum = resize_tensor(self.xyz_gradient_accum, (num_points, 1))
        self.denom = resize_tensor(self.denom, (num_points, 1))
        self.max_radii2D = resize_tensor(self.max_radii2D, (num_points,))

    def _resize_binding(self, num_points):
        if self.binding is None or self.binding.shape[0] == num_points:
            return
        if self.binding.shape[0] > num_points:
            self.binding = self.binding[:num_points]
        else:
            pad_count = num_points - self.binding.shape[0]
            fill_value = self.binding[-1:] if self.binding.numel() > 0 else torch.zeros(1, dtype=torch.int32, device="cuda")
            self.binding = torch.cat((self.binding, fill_value.repeat(pad_count)), dim=0)

        if self.binding_counter is not None:
            self.binding_counter = torch.zeros_like(self.binding_counter)
            self.binding_counter.scatter_add_(
                0,
                self.binding.long(),
                torch.ones(self.binding.shape[0], dtype=torch.int32, device=self.binding.device),
            )
    ...
    def create_from_pcd(self, pcd : Optional[BasicPointCloud], spatial_lr_scale : float):
        ...
        if self.binding is None:
            ...
            scales = torch.log(torch.ones((self._xyz.shape[0], 3), device="cuda"))
    ...
    def prune_points(self, mask):
        if mask.shape[0] != self._xyz.shape[0]:
            if mask.shape[0] < self._xyz.shape[0]:
                pad = torch.zeros(self._xyz.shape[0] - mask.shape[0], device=mask.device, dtype=mask.dtype)
                mask = torch.cat((mask, pad), dim=0)
            else:
                mask = mask[:self._xyz.shape[0]]
        self._resize_binding(self._xyz.shape[0])
        ...
        self._resize_densification_stats(valid_points_mask.shape[0])
        ...
        self._invalidate_cached_geometry()
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
        ...
        self._resize_binding(num_points)
