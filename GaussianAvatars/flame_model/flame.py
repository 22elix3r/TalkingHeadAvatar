...
FLAME_MESH_PATH = os.path.join(BASE_DIR, "assets/flame/head_template_mesh.obj")
FLAME_LMK_PATH = os.path.join(BASE_DIR, "assets/flame/landmark_embedding_with_eyes.npy")

# to be downloaded from https://flame.is.tue.mpg.de/download.php
# FLAME_MODEL_PATH = os.path.join(BASE_DIR, "assets/flame/generic_model.pkl")  # FLAME 2020
FLAME_MODEL_PATH = os.path.join(BASE_DIR, "assets/flame/flame2023.pkl")  # FLAME 2023 (versions w/ jaw rotation)
FLAME_PARTS_PATH = os.path.join(BASE_DIR, "assets/flame/FLAME_masks.pkl") # FLAME Vertex Masks
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        if flame_model_path.endswith(".safetensors"):
            data = safetensors_load(flame_model_path)
            ss = {k: v.cpu().numpy() for k, v in data.items()}
    ...
    def add_teeth(self):
        ...
        self.register_buffer(
            "upper_teeth_face_ids",
            torch.arange(num_faces_orig, num_faces_orig + f_teeth_upper.shape[0], dtype=torch.long),
            persistent=False,
        )
        self.register_buffer(
            "lower_teeth_face_ids",
            torch.arange(
                num_faces_orig + f_teeth_upper.shape[0],
                num_faces_orig + f_teeth_upper.shape[0] + f_teeth_lower.shape[0],
                dtype=torch.long,
            ),
            persistent=False,
        )
    ...
    def construct_mouth_structure(self, vertices: torch.Tensor | None = None):
        """
        Build structured inner-mouth mesh parts from existing FLAME+teeth topology.

        Returns:
            dict with:
              - upper_teeth_palate: (V, F)
              - lower_teeth_floor: (V, F)
              - frontal_teeth: (V, F)
              - molar_teeth: (V, F)
        """
        verts = self.v_template if vertices is None else vertices
        if verts.ndim == 3:
            verts = verts[0]

        if not hasattr(self, "upper_teeth_face_ids") or not hasattr(self, "lower_teeth_face_ids"):
            raise RuntimeError("Mouth structure requires add_teeth=True so teeth face IDs are available.")

        upper_face_ids = self.upper_teeth_face_ids
        lower_face_ids = self.lower_teeth_face_ids

        # Front-facing bands in add_teeth() are encoded as the first 56 triangles
        # of each upper/lower set; the remainder approximates molar/inner surfaces.
        frontal_ids = torch.cat([upper_face_ids[:56], lower_face_ids[:56]], dim=0)
        molar_ids = torch.cat([upper_face_ids[56:], lower_face_ids[56:]], dim=0)

        def extract_part(face_ids: torch.Tensor):
            faces = self.faces[face_ids]
            unique_vid = torch.unique(faces.reshape(-1), sorted=True)
            remapped = torch.searchsorted(unique_vid, faces.reshape(-1)).reshape(-1, 3)
            part_verts = verts[unique_vid]
            return part_verts, remapped

        upper_teeth_palate = extract_part(upper_face_ids)
        lower_teeth_floor = extract_part(lower_face_ids)
        frontal_teeth = extract_part(frontal_ids)
        molar_teeth = extract_part(molar_ids)

        return {
            "upper_teeth_palate": upper_teeth_palate,
            "lower_teeth_floor": lower_teeth_floor,
            "frontal_teeth": frontal_teeth,
            "molar_teeth": molar_teeth,
            "upper_face_ids": upper_face_ids,
            "lower_face_ids": lower_face_ids,
        }
